...
.PHONY: run-rest-api-server
run-rest-api-server: ## build rest api server and then run entrypoint.sh
	go build -o rest-api-server ./cmd/rest-api-server
	./entrypoint.sh rest-api-server

.PHONY: run-grpc-server
run-grpc-server: ## build grpc server and then run entrypoint.sh
	go build -o grpc-server ./cmd/grpc-server
	./entrypoint.sh grpc-server
